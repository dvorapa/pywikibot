    ...
    def treat_page(self):
        ...
        for part in pageParts:
            ...
            if part[-2:] == '}}' and (inBlockTemplate[-1] or inInlineTemplate[-1]):
                ...
                inTag.pop()
        vals = [inTag, inTable, inLink, inInlineTemplate, inBlockTemplate]
        if not all(v == [False] for v in vals):
            with open('soubor.txt', 'a') as soubor:
                soubor.write('# ' + self.current_page.title(asLink=True) + '\t' + ','.join(str(v) for v in vals) + '\n')
