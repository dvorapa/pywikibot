    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match('\{\{\s*[Cc]it(?:ace[ _](?:elektronické(?:ho|)[ _]|)(?:monografie|periodika)|[^aeáyu\/])', part):
                ...
                params = {'url_archivu':0,'datum_archivace':0,'nedostupne':0}
            elif part[:2] == '{{':
                inTemplate.append(1)
            elif part[:1] == '[':
                inLink.append(True)
            ...
            if inTemplate[-1] == 2 and not inLink[-1] and not inTag[-1]:
                ...
                params['url_archivu'] += len(re.findall(r'\|\s*(?:archive-?url|url archivu)\s*=', part, re.I))
                params['datum_archivace'] += len(re.findall(r'\|\s*(?:archive-?date|datum archivace)\s*=', part, re.I))
                params['nedostupne'] += len(re.findall(r'\|\s*(?:dead-?url|nedostupné)\s*=', part, re.I))

                part = textlib.replaceExcept(part, r'\|\s*archive-?url\s*=', r'| url archivu =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*archive-?date\s*=', r'| datum archivace =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*(dead-?url|nedostupné)\s*=\s*yes', r'| nedostupné = ano', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*(dead-?url|nedostupné)\s*=\s*no', r'| nedostupné = ne', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*dead-?url\s*=', r'| nedostupné =', self.vyjimky)
            ...
            if part[-2:] == '}}' and inTemplate[-1] > 0:
                if inTemplate[-1] == 2:
                    for value in params.values():
                        if value > 1:
                            with open('duplicity.txt', 'a') as soubor:
                                soubor.write('# ' + self.current_page.title(as_link=True) + '\n')
                            return
                inTemplate.pop()
