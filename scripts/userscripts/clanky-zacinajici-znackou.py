...
class BasicBot(
    ...
    def treat_page(self):
        ...
        predloha = r'(?:\s*(?:{{[^{}]*}}|\[\[\s*(?:' + r'|'.join(pywikibot.Site('cs').namespaces.FILE) + r')[^\[\]]*\]\]|<!--(?:[^-]|-[^-]|--[^>])-->))*\s*<[^!]'
        if re.match(predloha, text):
            self.seznam += '# ' + self.current_page.title(as_link=True) + '\n'
...
def main(*args):
    ...
    if gen:
        ...
        page = pywikibot.Page(pywikibot.Site('cs'), 'Wikipedie:Údržbové seznamy/Články začínající značkou/seznam')
