...
class BasicBot(
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *Infobox[ _]Sezona[ _]fotbalový[ _]klub', part, flags=re.I):
                inBlockTemplate.append(True)
                part = textlib.replaceExcept(part, r'\{\{ *[Ii]nfobox[ _]Sezona[ _]fotbalový[ _]klub', r'{{Infobox - sezona fotbalového klubu', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*výs(\d)\s*=', r'| výsledek\1 =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*nejstřelec\s*=', r'| nejlepší střelec =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\s*\|\s*(?:linka|ročník)\s*=[^\n]*', r'', self.vyjimky)
