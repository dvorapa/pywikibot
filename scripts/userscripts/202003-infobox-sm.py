    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(self.infobox, part):
                ...
                part = textlib.replaceExcept(part, self.infobox, r'{{Infobox - česká obec', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*nuts4\s*=', r'| LAU1 =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*nuts5\s*=', r'| LAU2 =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*nuts3\s*=', r'| NUTS3 =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*ob\.roz\.půs\s*=', r'| obec s rozšířenou působností =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*pov\.ob\s*=', r'| pověřená obec =', self.vyjimky)
                ...
                part = textlib.replaceExcept(part, r'\|\s*rozpočet\s*=', r'| rozpočtové výdaje =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*rozpočet aktuální k\s*=', r'| rozpočtové výdaje aktuální k =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*počet zsj\s*=', r'| počet ZSJ =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*počet částí\s*=', r'| počet částí obce =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*počet obvodů\s*=', r'| počet městských obvodů =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*počet adres\s*=', r'| počet adresních míst =', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*počet obyvatel\s*=[^\n]*\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*obyvatelé aktuální k\s*=[^\n]*\n', r'', self.vyjimky)
            ...
            if part[-2:] == '}}' and (inBlockTemplate[-1] or inInlineTemplate[-1]):
                if inBlockTemplate[-1] and inInlineTemplate[-1]:
                    ...
                    inBlockTemplate.pop()
                    part = textlib.replaceExcept(part, r'\}\}$', r' | status = statutární město\n}}', self.vyjimky)
        ...
        text = textlib.replaceExcept(text, r' *\|\s*loc-map\s*=[^\n]*\n', r'', self.vyjimky, count=1)
