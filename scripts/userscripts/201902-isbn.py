...
class BasicBot(
    ...
    def treat_page(self):
        ...
        pat = r'([^;\'\s=\.\<,\:\)]+)'
        text = textlib.replaceExcept(text, r'((?:\<nowiki\>\s*)?[Ii][Ss][Bb][Nn][^\d=]*)' + pat + r'((?:\s*\<\/nowiki\>)?)', lambda x: x.group(1) + ''.join(ch.upper() for ch in x.group(2) if ch.isdigit() or ch.upper() == 'X') + x.group(3), self.vyjimky)
        text = textlib.replaceExcept(text, r'(?:\<nowiki\>\s*)?[Ii][Ss][Bb][Nn][^\d=]*' + pat + r'(?:\s*\<\/nowiki\>)?', r'{{ISBN|\1}}', self.vyjimky)
