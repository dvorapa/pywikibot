...
class BasicBot(
    ...
    def treat_page(self):
        ...
        for n, part in enumerate(pageParts):
            if re.match(self.infobox, part):
                inTemplate.append(2)
                part = textlib.replaceExcept(part, self.infobox, r'{{Infobox - české sídlo', self.vyjimky)
            elif part[:2] == '{{':
                inTemplate.append(1)
            elif part[:1] == '[':
                inLink.append(True)
            ...
            if inTemplate[-1] == 2 and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*město\s*=', r'| obec =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*obvod\s*=', r'| městská část =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*výměra\s*=', r'| rozloha =', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*počet obyvatel\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*počet domů\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*domy aktuální k\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
                if re.search(r'\|\s*poznámka\s*=', part, re.I):
                    pozn += 1
                    part = textlib.replaceExcept(part, r' *\|\s*poznámka\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*kód ZSJ\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r' *\|\s*zanikleobce\s*=\s*[^\n\}\|]*\n', r'', self.vyjimky)
        ...
        text = textlib.replaceExcept(text, r' *\|\s*loc-map\s*=\s*([^\}\n]*\}\})? *\n', r'', self.vyjimky, count=1)
        text = textlib.replaceExcept(text, r' *\|\s*obyvatelé aktuální k\s*=([^\n\<]*\<[^\/]*\/[^\>]*\>)?[^\n]*\n', r'', self.vyjimky, count=1)

        # if summary option is None, it takes the default i18n summary from
        # i18n subdirectory with summary_key as summary key.
        if pozn > 0:
            with open('pozn.txt', 'a') as soubor:
                soubor.write('# ' + self.current_page.title(as_link=True) + '\n')
