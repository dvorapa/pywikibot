...
basic.py
        in this case). Use summary_key to set a default edit summary 
message.
...
pages
    ...
    def _handle_dry_param(self, **kwargs):
        ...
        This is a private method. It prints a deprecation warning for 
old
    ...
    def treat_page(self):
        ...
        pat = r'([^\s\)\.<:,;]+)'
        while True:
            m = re.search(r'isbn', text[pos:], re.I)
            if not m:
                break
            if re.search(r'(?:\||\{\{)\s*$', text[:pos+m.start()]) and re.match(r'\d*\s*[=\|]', text[pos+m.end():]):
                pos += m.end()
                continue
            text = re.sub(r'<\s*nowiki\s*>\s*$', r'', text[:pos+m.start()], re.I) + '{{ISBN|' + re.sub(r'^[\s:]*' + pat + r'(?:\s*<\s*/\s*nowiki\s*>)?', r'\1}}', text[pos+m.end():], re.I)
            pos += m.end()
