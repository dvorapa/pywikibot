...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'doplnění navigační šablony'
    ...
    def treat_page(self):
        ...
        navbox = r'{{Pilotované lety na ISS}}'
        if re.search(r'\{\{ *(?:[Pp]ortály|[Aa]utoritní[ _]data|DEFAULTSORT)', text):
            text = textlib.replaceExcept(text, r'(\{\{ *(?:[Pp]ortály|[Aa]utoritní[ _]data|DEFAULTSORT))', navbox + r'\n\n\1', self.vyjimky, count=1)
        elif re.search(r'\[\[ *' + r'|'.join(self.site.namespaces[14]) + r' *:', text):
            text = textlib.replaceExcept(text, r'(\[\[ *' + r'|'.join(self.site.namespaces[14]) + r' *:)', navbox + r'\n\n\1', self.vyjimky, count=1)
        else:
            text += r'\n\n' + navbox
