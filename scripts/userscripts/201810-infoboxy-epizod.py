    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *Infobox[ _]', part, flags=re.I):
                ...
                part = textlib.replaceExcept(part, r'Infobox - epizoda Futuramy', r'Infobox - díl seriálu Futurama', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                # part = textlib.replaceExcept(part, r'', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*číslo epizody\s*=', r'| číslo dílu =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*vysíláno\s*=', r'| premiéra =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*autor\s*=', r'| scénář =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*sezóna\s*=', r'| řada =', self.vyjimky)
