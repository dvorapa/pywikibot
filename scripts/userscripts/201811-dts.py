...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'oprava rozbité šablony; aktualizace komentáře'
    ...
    def treat_page(self):
        ...
        text = textlib.replaceExcept(text, r'\s*<!--\s*den\s*\|\s*měsíc\s*\|\s*rok\s*-->\s*', r' <!-- rok|měsíc|den -->\n', self.vyjimky)

        pattern = re.compile(r'\{\{subst:Datum([^\}]*)\}\}')
        nález = pattern.search(text)
        úprava = ''
        parametry = []
        while nález:
            úprava = text[nález.start(1):nález.end(1)] + '~'
            parametry = re.findall(r'\|[^\|\=]*(?=[\|\}\~])', úprava)
            parametry.reverse()
            text = text[:nález.start(1)-11] + 'Dts' + ''.join(parametry) + text[nález.end(1):]
            nález = pattern.search(text, nález.end(1))
