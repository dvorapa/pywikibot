...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'oprava duplicitního parametru, překlad parametrů'
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *Citace[ _]', part, flags=re.I):
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                # part = textlib.replaceExcept(part, r'', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *archivedate *=\s*', r'|datum archivace = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *url *=\s*(?:https?:)?\/\/(?:web|wayback)\.archive\.org\/(?:web\/)?', r'|url archivu = http://web.archive.org/web/', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *url *=\s*(?:https?:)?\/\/(?:www\.)?archive\.is\/', r'|url archivu = https://archive.is/', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *deadurl *=\s*no', r'|nedostupné = ne', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *deadurl *=\s*yes', r'|nedostupné = ano', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *year *=\s*', r'|rok = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *lang *=\s*', r'|jazyk = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *location *=\s*', r'|místo = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *month *=\s*', r'|měsíc = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *first(\d+) *=\s*', r'|jméno\1 = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\| *last(\d+) *=\s*', r'|příjmení\1 = ', self.vyjimky)
