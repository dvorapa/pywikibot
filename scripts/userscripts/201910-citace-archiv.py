...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'oprava url v referenci'
    ...
    def treat_page(self):
        ...
        text = textlib.replaceExcept(text, r'\|\s*url\s*=\s*https?:\/\/(?:web\.)?archive\.org\/web\/(\d+)\/([^\s\|\}]*)', r'| url = \2 | url archivu = https://web.archive.org/web/\1/\2', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*url\s*=\s*([^\s\|\}]*)([^\{\}]*)\|\s*url\s*=\s*\1', r'| url = \1\2', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*(?:archiveurl|archive-url|url archivu)\s*=\s*([^\s\|\}]*)([^\{\}]*)\|\s*(?:archiveurl|archive-url|url archivu)\s*=\s*\1', r'| url archivu = \1\2', self.vyjimky)
