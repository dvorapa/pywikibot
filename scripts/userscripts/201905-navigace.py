...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.availableOptions.update({
            ...
            'titul': '',  # append text on top of the page
            'autor': '',  # append text on top of the page
        })
    ...
    def treat_page(self):
        ...
        if re.search('\{\{\s*[Nn]avigacePaP', text):
            return False
        ...
        prefix, slash, chapter = self.current_page.title().rpartition('/')
        if not slash:
            return False
        with open('lekek.txt') as lines:
            lines = list(lines)
        lines = [line.rstrip() for line in lines]
        linecount = len(lines)
        pre = '{{NavigacePaP\n  | TITUL = ' + self.getOption('titul') + '\n  | AUTOR = ' + self.getOption('autor')
        inside = ''
...
def main(*args):
    ...
    for arg in local_args:
        ...
        option = arg[1:]
        if option in ('summary', 'text', 'titul', 'autor'):
            if not value:
