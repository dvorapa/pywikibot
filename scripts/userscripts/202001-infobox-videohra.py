...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        shrnuti = 'aktualizace parametrů infoboxu videohra dle [[Wikipedie:Bot/Žádosti_o_provedení_práce#Infobox_-_videohra|žádosti]] z 10. 1. 2020'
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*(?:CERO|ESRB|GSRR|USK|ředitel|distributor|distributor v čr|licence|hodnocení|rozlišení|počet barev|poměr stran|verze|média|požadavky|ovládání|ratingy|PEGI)\s*= *\n', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*série\s*=', r'| herní série =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*platformy\s*=', r'| platforma =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*módy\s*=', r'| herní mód =', self.vyjimky)
