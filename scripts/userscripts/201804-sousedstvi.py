...
class BasicBot(
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *(Sousedstvi|Geografické[ _]okolí2)', part, flags=re.I):
                inBlockTemplate.append(True)
                part = textlib.replaceExcept(part, r'\{\{ *(Sousedstvi|Geografické[ _]okolí2)', r'{{Geografické okolí', exceptions)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                part = textlib.replaceExcept(part, r'(\|\s*[A-ZŽŠČŘĎŤŇÁÉÍÓÚŮÝĚ])([A-ZŽŠČŘĎŤŇÁÉÍÓÚŮÝĚ]*)', lambda x: x.group(1) + x.group(2).lower(), exceptions)
                part = textlib.replaceExcept(part, r'\|[^\|\=]+?=', lambda x: x.group(0).replace('_',' '), exceptions)
                ################################################################
                #                            regexy                            #
                ################################################################
                ...
                part = textlib.replaceExcept(part, r'\|\s*Textneboobraz', r'| Střed', exceptions)
