    ...
    def treat_page(self):
        ...
        for part in pageParts:
            ...
            if re.match(name, part):
                ...
                # part = textlib.replaceExcept(part, name, r'', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*css\s*=', r'| proměnná CSS =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*(rgb|cmyk|hsv|lab)\s*=\s*\(([^\)]+)\)', r'| \1 = \2', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*hex\s*=\s*\#', r'| hex = ', self.vyjimky)
            ...
            if part[-2:] == '}}' and (inBlockTemplate[-1] or inInlineTemplate[-1]):
                if inBlockTemplate[-1] and inInlineTemplate[-1]:
                    ...
                    inBlockTemplate.pop()
                    newPageParts.append('[[Kategorie:Barvy]]')
