...
class BasicBot(
    ...
    def treat_page(self):
        ...
        exceptions = ['comment', 'header', 'noinclude', 'includeonly', 'templatedata']
        ...
        exc_regexes.append(re.compile('(?is)\{\{ *(Dokumentace|Documentation|Dlouhodobě polozamčeno|Dlouhodobě zamčeno)[^\}]*\}\}', re.I))
        exc_regexes.append(re.compile('(?is)\[\[ *Kategorie:Wikipedie:Netisknout[^\]]*\]\]', re.I))
        for regex in exc_regexes:
            for match in re.finditer(regex, text):
                ...
                text = before + after
        if text.strip() == '|}':
            with open('konec-tabulky.txt', 'a') as konec_tabulky:
                konec_tabulky.write('# ' + self.current_page.title(asLink=True) + '\n')
