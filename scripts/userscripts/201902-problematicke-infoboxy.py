...
class BasicBot(
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *Infobox[ _]-[ _]sídlo[ _]světa', part, flags=re.I):
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                if not re.search(r'\|\s*(?:název|name|jméno)\s*=\s*' + re.escape(title) + r'\s*[\|\}]', part) and re.search(r'\|\s*(?:název|name|jméno)\s*=\s*[^\s\|\}]', part):
                    with open('soubor.txt', 'a') as soubor:
