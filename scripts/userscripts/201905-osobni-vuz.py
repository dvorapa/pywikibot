...
class BasicBot(
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{\s*Infobox[ _]Osobní[ _]vůz[ _]ČD', part, flags=re.I):
                inBlockTemplate.append(True)
                part = textlib.replaceExcept(part, r'\{\{\s*Infobox[ _]Osobní[ _]vůz[ _]ČD', r'{{Infobox - osobní vůz', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*(řada|číselné označení) vozu\s*=\s*', r'| \1 = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*obrázek najbrt\s*=\s*', r'| bokorys najbrt = ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*provozované v současnosti\s*=\s*', r'| počet provozovaných kusů = ', self.vyjimky)
            ...
            if part[-2:] == '}}' and (inBlockTemplate[-1] or inInlineTemplate[-1]):
                if inBlockTemplate[-1] and inInlineTemplate[-1]:
                    ...
                    part = textlib.replaceExcept(part, r'\}\}', r' | domovský stát = Česko\n}}', self.vyjimky)
                    inBlockTemplate.pop()
