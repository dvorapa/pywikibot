...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'standardizace parametrů infoboxu'
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            ...
            if re.match(name, part, flags=re.I):
                ...
                #part = textlib.replaceExcept(part, name, r'{{Infobox - správní obvod Anglie', self.vyjimky)
            elif part[:2] == '{{':
        ...
        text = textlib.replaceExcept(text, r'\|\s*znak\s*=\s*(?:Soubor|File|Image|Datei)\s*:', r'| znak = ', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*vlajka\s*=\s*(?:Soubor|File|Image|Datei)\s*:', r'| vlajka = ', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*mapa\s*=\s*(?:Soubor|File|Image|Datei)\s*:', r'| mapa = ', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*mapa[ _]okresu\s*=\s*(?:Soubor|File|Image|Datei)\s*:', r'| mapa okresu = ', self.vyjimky)
        #text = textlib.replaceExcept(text, r'\|\s*ikona\s*=\s*\[\[\s*(?:Soubor|File|Image|Datei)\s*:\s*([^\|\}]+)[^\]]*\]\]', r'| ikona = \1', self.vyjimky)
        #text = textlib.replaceExcept(text, r'\|\s*logo\s*=\s*\[\[\s*(?:Soubor|File|Image|Datei)\s*:\s*([^\|\}]+)[^\]]*\|([^\]]+)\]\]', r'| logo = \1\n | popisek loga = \2', self.vyjimky)
        #text = textlib.replaceExcept(text, r'\|\s*popisek loga\s*=\s*\d+\s*px', r'| popisek loga = ', self.vyjimky)
        #text = textlib.replaceExcept(text, r'\|\s*popisek loga\s*=\s*upright=[\d\.]*', r'| popisek loga = ', self.vyjimky)
