...
class BasicBot(
    ...
    def treat_page(self):
        ...
        infobox = '''{{{{Infobox - sezóna hokejového klubu
 | název klubu = [[HC Oceláři Třinec]]
 | sezóna = {sezona}
 | obrázek = 
 | popisek = 
 | soutěže = [[Extraliga ledního hokeje]]
 | manažer = 
 | trenér = 
 | kapitán = 
 | sponzor = 
 | předchozí = [[HC Oceláři Třinec v české hokejové extralize {zpet}|{zpet}]]
 | následující = [[HC Oceláři Třinec v české hokejové extralize {dal}|{dal}]]
}}}}'''
        subst, clanek = text.split('|}', 1)
        try:
            nalez = re.search(r"''' *(\d+)\/(\d+) *'''", subst)
            rok1 = nalez.group(1)
            rok2 = nalez.group(2)
            ...
            return
        sezona = rok1 + '/' + rok2
        zpet = str(int(rok1) - 1) + '/' + rok1
        dal = rok2 + '/' + str(int(rok2) + 1)
        infobox = infobox.format(sezona=sezona, zpet=zpet, dal=dal)
        text = infobox + clanek
