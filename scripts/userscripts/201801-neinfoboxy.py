...
class BasicBot(
    ...
    def treat_page(self):
        ...
        exceptions = ['nowiki', 'noinclude', 'comment', 'source', 'pre', 'syntaxhighlight', 'code']

        ################################################################
        #                           shrnutí                            #
        ################################################################

        #shrnuti = 'parametry přejmenovány'
        ...
        parametry = re.findall(r'\{{3}', text)
        if len(parametry) < 6:
            with open('neinfoboxy.txt', 'a') as neinfoboxy:
                neinfoboxy.write('# ' + self.current_page.title(asLink=True) + '\n')
