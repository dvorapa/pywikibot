...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'oprava pořadí parametrů šablony'
    ...
    def treat_page(self):
        ...
        pattern = re.compile(r'\{\{ *[Jj]ULGREGDATUM([^\}]*)\}\}')
        ...
        while nález:
            ...
            if len(parametry) == 3:
                ostatní = re.findall(r'\|[^\|\=]*=[^\|]*(?=[\|\}~])', úprava)
                parametry.reverse()
                text = text[:nález.start(1)] + ''.join(parametry) + ''.join(ostatní) + text[nález.end(1):]
            else:
                print(self.current_page.title(with_ns=True))
