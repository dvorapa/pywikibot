...
class BasicBot(
    ...
    def treat_page(self):
        ...
        casti = self.current_page.title().split(' ')
        posledni = casti[len(casti)-1]
        text = textlib.replaceExcept(text, r'\[\[ *([Kk]ategorie|[Cc]ategory) *: *Seznamy měst v evropských zemích *\]\]', r'[[Kategorie:Seznamy měst v evropských zemích|' + posledni + r']]', exceptions)
