...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        shrnuti = 'sloučení zastaralého infoboxu'

        ################################################################

        self.shrnuti = self.getOption('summary') or 'Robot: ' + shrnuti

        infobox = self.getOption('ref') or 'Šablona:Infobox - české sídlo'
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            ...
            if inTemplate[-1] == 2 and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                part = textlib.replaceExcept(part, r'\|\s*městské části\s*=', r'| městská část =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*správní obvody\s*=', r'| správní obvod =', self.vyjimky)
            ...
            if part[-2:] == '}}' and inTemplate[-1] > 0:
                if inTemplate[-1] == 2:
                    part = textlib.replaceExcept(part, r'\}\}$', r' | obec = [[Praha]]\n}}', self.vyjimky)
                inTemplate.pop()
        ...
        text = textlib.replaceExcept(text, r'\n *\|\s*(?:znak|vlajka|datum připojení|městské obvody|počet útj|hustota|kód kú|počet obyvatel|obyvatelé aktuální k)\s*=[^\n]*', r'', self.vyjimky)
