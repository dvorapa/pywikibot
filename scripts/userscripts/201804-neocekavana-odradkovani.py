...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.availableOptions.update({
            'priprava': False,  # delete old text and write the new text
    ...
    def treat_page(self):
        ...
        shrnuti = '-neočekávané odřádkování'
        ...
        if self.getOption('priprava'):
            if re.search(r'\s+\<(noinclude|includeonly|\/onlyinclude)\>', text):
                with open('odradkovani.txt', 'a') as odradkovani:
                    odradkovani.write('# ' + self.current_page.title(asLink=True) + '\n')
        else:
            text = textlib.replaceExcept(text, r'\s+\<(noinclude|includeonly|\/onlyinclude)\>', r'<\1>', exceptions)
