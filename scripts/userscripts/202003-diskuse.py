...
class BasicBot(
    ...
    def treat_page(self):
        ...
        count = len(extract_sections(text)[1])
        size = len(text)
        if count >= 30 and size >= 50000:
            with open('oboji.txt', 'a') as soubor:
                soubor.write('# ' + self.current_page.title(as_link=True) + ': ' + str(size) + ' (' + str(count) + ')\n')
        elif count >= 30:
            with open('sekce.txt', 'a') as soubor:
                soubor.write('# ' + self.current_page.title(as_link=True) + ': ' + str(size) + ' (' + str(count) + ')\n')
        elif size >= 50000:
            with open('velikost.txt', 'a') as soubor:
                soubor.write('# ' + self.current_page.title(as_link=True) + ': ' + str(size) + ' (' + str(count) + ')\n')
