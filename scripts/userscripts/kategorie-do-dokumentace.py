...
class BasicBot(
    ...
    def treat_page(self):
        ...
        shrnuti = 'vyjmutí kategorií do dokumentace'
        ...
        kategorie = [re.sub(r'<\/?noinclude>', r'', s).strip() for s in kategorie]
        kategorie = '\n'.join(kategorie)
        kategorie = re.findall(r'\[\[ *Kategorie:[^\]]+\]\]', kategorie, flags=re.I)
        ...
        text = self.current_page.text
        text += '\n\n<includeonly>\n' + '\n'.join(kategorie) + '\n</includeonly>'
        text = textlib.replaceExcept(text, r'\s*<\/includeonly>\s*<includeonly>\s*', r'\n', exceptions)
