...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'rozdělení šablony na navbox a infobox'
    ...
    def treat_page(self):
        ...
        text = textlib.replaceExcept(text, r'\{\{\s*[Aa]lpské[ _]lyžování[ _]na[ _]ZOH[ _]2018\s*\|', r'{{Alpské lyžování na ZOH 2018}}\n{{Infobox - olympijská disciplína\n |', self.vyjimky)
        text = textlib.replaceExcept(text, r'\|\s*disciplína\s*=\s*([^&]+)&nbsp;•&nbsp;([^&]+)&nbsp;•&nbsp;([^\n]+)', r'| disciplína = \2\n | hry = 2018 zimní\n | předchozí = \1\n | následující = \3', self.vyjimky)
        ...
        for part in pageParts:
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                # part = textlib.replaceExcept(part, r'', r'', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*areál\s*=', r'| dějiště =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*startujících\s*=\s*(\d+)[^\n]*', r'| počet účastníků = \1', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*[Nn][Oo][Vv]\s*=\s*(\d+)[^\n]*', r'| počet zemí = \1', self.vyjimky)
