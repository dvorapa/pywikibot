...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'oprava parametru infoboxu'
    ...
    def treat_page(self):
        ...
        find = re.findall(r'\|\s*[Ss]oučást[ _]státu\s*=\s*[^\|\}]*', text)
        if len(find) > 1:
            find = re.search(r'\n *\|\s*[Ss]oučást[ _]státu\s*=\s*([^\|\}]*)', text)
            text = textlib.replaceExcept(text, r'\n *\|\s*[Ss]oučást[ _]státu\s*=\s*[^\|\}]*\s*', r'\n', self.vyjimky, count=1)
            find = find.group(1).strip()
            text = textlib.replaceExcept(text, r'(\|\s*[Ss]oučást[ _]státu\s*=\s*)([^\|\}]*)', r'\1' + find + r', \2', self.vyjimky)
