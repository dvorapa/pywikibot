...
class BasicBot(
    ...
    def treat_page(self):
        ...
        text = re.sub(r'\<\s*references\s*/\s*\>', r'', text, re.I)
        ...
        if re.search(r'\{\{ *[Pp]řeklad[^\}]*\}\}', text):
            ...
            if re.search(r'=== *[Pp]oznámky *===', text):
                text = textlib.replaceExcept(text, r'(\<references +group[^\>]*\>)', r'\1\n\n' + sekce3, self.vyjimky)
            else:
                text = textlib.replaceExcept(text, r'(\<references +group[^\>]*\>)', r'\1\n\n' + sekce, self.vyjimky)
        elif re.search(r'== *[Oo]dkazy *==', text):
            text = textlib.replaceExcept(text, r'(== *[Oo]dkazy *==)', r'\1\n' + sekce3, self.vyjimky)
        elif re.search(r'== *(?:[Ll]iteratura|[Ss]ouvisející články|[Ee]xterní odkazy) *==', text):
            if re.search(r'=== *(?:[Ll]iteratura|[Ss]ouvisející články|[Ee]xterní odkazy) *===', text):
            ...
            for sablona in sablony:
                ...
                if re.search(r'\{\{ *(?:[Pp]ortály|[Aa]utoritní[ _]data|DEFAULTSORT)', text):
                    ...
                    text = textlib.replaceExcept(text, r'(\[\[ *' + r'|'.join(self.site.namespaces[14]) + r' *:)', sekce + r'\n\n\1', self.vyjimky, count=1)
                else:
                    text += r'\n\n' + sekce
