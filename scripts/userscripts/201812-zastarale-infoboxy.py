...
class BasicBot(
    ...
    def __init__(self, generator, **kwargs):
        ...
        self.shrnuti = 'vhodnější infobox'
    ...
    def treat_page(self):
        ...
        for part in pageParts:
            if re.match(r'\{\{ *Infobox[ _]', part, flags=re.I):
                ...
                # part = textlib.replaceExcept(part, r'\{\{ *Infobox[ _]', r'{{Infobox - ', self.vyjimky)
                part = textlib.replaceExcept(part, r'\{\{ *Infobox[ _]-[ _]opakující[ _]se[ _]událost', r'{{Infobox - sportovní akce', self.vyjimky)
            elif part[:2] == '{{':
            ...
            if inBlockTemplate[-1] and not inInlineTemplate[-1] and not inLink[-1] and not inTable[-1] and not inTag[-1]:
                ...
                # match = re.search(r'\|\s*název\s*=\s*([^\|\}]+?)\s*[\|\}]', part)
                # if match:
                #     kde = match.end() - 1
                #     co = match.group(1)
                # match2 = re.search(r'\|\s*název\s*=\s*' + re.compile(co).pattern + r'\s*[\|\}]', part[kde:])
                # if match2:
                #     part = part[:kde + match2.start()] + part[kde + match2.end() - 1:]
                # part = textlib.replaceExcept(part, r'\|\s*\s*=', r'|  =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*téma\s*=', r'| sport =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*datum\s*=', r'| datum konání =', self.vyjimky)
                part = textlib.replaceExcept(part, r'\|\s*účast\s*=', r'| účastníci =', self.vyjimky)
        ...
        text = textlib.replaceExcept(text, r'{{Infobox - sportovní akce\s*}}', r'{{Infobox - opakující se událost}}', self.vyjimky)
