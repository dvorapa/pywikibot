...
class BasicBot(
    ...
    def treat_page(self):
        ...
        text = textlib.replaceExcept(text, r'\s*\|\s*pečeť\s*=[^\n]*', r'', self.vyjimky, count=1)
        text = textlib.replaceExcept(text, r'\s*\|\s*počet domů\s*=[^\n]*', r'', self.vyjimky, count=1)
        text = textlib.replaceExcept(text, r'\s*\|\s*počet obyvatel\s*=[^\n]*', r'', self.vyjimky, count=1)
        text = textlib.replaceExcept(text, r'\s*\|\s*obyvatelé aktuální k\s*=[^\n]*', r'', self.vyjimky, count=1)
        text = textlib.replaceExcept(text, r'\s*\|\s*loc-map\s*=[^\n]*', r'', self.vyjimky, count=1)
